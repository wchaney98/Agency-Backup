...
namespace SplatterSystem {
    ...
    public static class BitmapDraw {
        ...
        public static void DrawFilledSquare(Texture2D texture, int x, int y, int sideSize, Color color) {
            ...
            int sideWidth = Mathf.Min(sideSize * 2, texture.width - x);
            int sideHeight = Mathf.Min(sideSize * 2, texture.height - y);

            int numPixels = sideWidth * sideHeight;
            if (colorsArray == null || colorsArray.Length < numPixels || 
                    colorsArray.Length <= 0 || colorsArray[0] != color) {
                colorsArray = new Color32[numPixels];
                for (int i = 0; i < colorsArray.Length; i++) {
                    colorsArray[i] = color;
                }
            }

            texture.SetPixels32(x, y, sideWidth, sideHeight, colorsArray);
        }
        ...
        public static void DrawLine(Texture2D texture, int x0, int y0, int x1, int y1, Color color) {
            ...
            for (int x = x0; x < x1; x++) {
                // Unity bug: if pixel (1, 1) is updated, the texture is cleared.
                if (y == 1 && x == 1) continue;
